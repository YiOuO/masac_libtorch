...
public:
	...
	torch::Tensor getGlobalState() {
		torch::Tensor state = torch::zeros({1, 4 * numberOfAgents_}, torch::kF64);
		for (int i = 0; i < numberOfAgents_; ++i) {
			state[0][4 * i + 0] = positions_[i][0];
			state[0][4 * i + 1] = positions_[i][1];
			state[0][4 * i + 2] = goals_[i][0];
			state[0][4 * i + 3] = goals_[i][1];
		}
		return state;
	}

	auto Act(const torch::Tensor& jointAction) -> std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, int>
	{
		old_dist_ = GoalDist(pos_);
	}

    // // Local observation of agent i: [pos_x, pos_y, goal_x, goal_y]
    // torch::Tensor getLocalObservation(int agentIndex) const {
    //     torch::Tensor obs = torch::zeros({1, 4}, torch::kF64);
    //     double buffer[4] = {
    //         positions_, positions_,
    //         goals_,     goals_
    //     };
    //     std::memcpy(obs.data_ptr(), buffer, 4 * sizeof(double));
    //     return obs;
    // }

    // // Global state: concatenation of all agents' local observations in fixed order
    // torch::Tensor getGlobalState() const {
    //     torch::Tensor state = torch::zeros({1, 4 * numberOfAgents_}, torch::kF64);
    //     double* ptr = state.data_ptr<double>();
    //     for (int i = 0; i < numberOfAgents_; ++i) {
    //         *ptr++ = positions_;
    //         *ptr++ = positions_;
    //         *ptr++ = goals_;
    ...
    // One environment step with the joint action [a1_x, a1_y, a2_x, a2_y, ...]
    // Returns: next_state, reward_vector (shape [1, N]), done_flag([1,1]), status_code
    ...
    step(const torch::Tensor& jointAction) {
        ...
        std::vector<double> previousDistance(numberOfAgents_, 0.0);
        for (int i = 0; i < numberOfAgents_; ++i) {
            previousDistance[i] = distance(positions_[i], goals_[i]);
        }
        ...
        for (int i = 0; i < numberOfAgents_; ++i) {
            ...
            double reward_i = 10.0 * (previousDistance[i] - distNow);  // distance reduction
    ...
    int numberOfAgents() const { return numberOfAgents_; }

    // Public for logging convenience (keep simple demo style)
    std::vector<Eigen::Vector2d> positions_;
    std::vector<Eigen::Vector2d> goals_;



    static double distance(const Eigen::Vector2d& a, const Eigen::Vector2d& b) {
        return (a - b).norm();
    }
};
